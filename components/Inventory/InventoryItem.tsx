...
export default function InventoryItem({ item }: InventoryItemProps) {
  ...
  return (
    <div
      key={item.key}
      className={`p-2 rounded border border-gray-100 ${item.quantity === 0 ? 'bg-gray-50' : ''}`}
    >
      <div className="flex items-center gap-1">
        <div className="text-lg">{item.icon}</div>
        <div className="flex-1 min-w-0">
          <div className={`text-xs font-medium ${item.quantity === 0 ? 'text-gray-400' : 'text-gray-800'} truncate`}>
            {item.name}
          </div>
          <div className={`text-xs ${item.quantity === 0 ? 'text-gray-400' : 'text-gray-500'}`}>
